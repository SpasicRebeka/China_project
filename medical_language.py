...
from config import PROJECT_ROOT, get_language_reasoning_effort, get_text_model
...
def _format_glossary_context(terms: list[GlossaryEntry] | list[SimpleTerm]) -> str:
    if not terms:
        return "- No local glossary terms matched."
    return "\n".join(f"- {term.term}: {term.plain_language}" for term in terms)
...
class MedicalLanguageProcessor:
    ...
    def __init__(
        ...
        self.reasoning_effort = get_language_reasoning_effort()
    ...
    def simplify_for_patient(
        ...
        """Translate if needed and create a patient-friendly explanation."""
        matched_terms = retrieve_glossary_terms(original_text, self.glossary)

        # Chinese or other non-English input will not match an English glossary
        # before translation. For the small prototype glossary, provide the
        # approved terms as context and filter the final term list afterward.
        glossary_context_terms = matched_terms
        if not glossary_context_terms and source_language.lower() not in {"english", "en"}:
            glossary_context_terms = self.glossary

        glossary_context = _format_glossary_context(glossary_context_terms)
        english_context = english_translation or "Translate the original text to English."
        ...
        response = self.client.responses.parse(
            ...
            instructions=(
                ...
                "First translate the original text to English if needed. Use short sentences and common words. "
                "Preserve all medication names, measurements, numbers, "
            ...
            input=(
                ...
                f"Original text:\n{original_text}\n\n"
                f"English translation guidance:\n{english_context}\n\n"
        ...
        post_translation_terms = retrieve_glossary_terms(result.english_translation, self.glossary)
        merged_terms = _merge_terms(result.simple_terms, post_translation_terms)
    ...
    def process(self, text: str, source_language: str = "auto") -> MedicalLanguageResult:
        """Run the low-latency translate + simplify cloud pipeline."""
        return self.simplify_for_patient(original_text=text, source_language=source_language)
